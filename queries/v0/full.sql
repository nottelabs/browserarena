...
metric_long AS (
  ...
  UNION ALL
  SELECT provider, concurrency, 'page_script_ms'             AS metric, page_script_ms             AS value FROM pre_success WHERE page_script_ms IS NOT NULL AND page_script_ms > 0
  UNION ALL
...
ORDER BY
  ...
  CASE ms.metric
    ...
    WHEN 'page_script_ms'             THEN 4
    WHEN 'page_ttfb_ms'               THEN 5
    WHEN 'page_dom_content_loaded_ms' THEN 6
    WHEN 'page_load_ms'               THEN 7
    WHEN 'session_release_ms'         THEN 8
    WHEN 'cost_usd'                   THEN 9
    WHEN 'total_ms'                   THEN 10
    ELSE 99
  END;
